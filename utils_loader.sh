DFLOADER() {
    if [[ $# -lt 1 ]]; then
        echo "Usage: DFLOADER <load|unload|list> [UTILITY_NAME]..." >&2
        return 1
    fi
    ...
    while [[ "$#" -gt 1 ]]; do
        ...
        local util_name="$1"
        local utils_dir="${DOTFILES_HOME}/utils"
        local util_file="${utils_dir}/${util_name}"

        if [[ ! -e "${util_file}" ]]; then
            echo "Utility '${util_name}' not found in ${utils_dir}." >&2
            return 1
        fi
...
' "${util_file}" 2>/dev/null)
    ...
    print;
        ...
        if [[ "$mode" == "load" ]]; then
            if [[ ":${_DF_LOADED_MODULES}:" == *":${util_name_clean}:"* ]]; then
                echo "Utility '${util_name}' is already loaded."
                continue
            fi
            ...
            if [[ ":${_DF_LOADED_MODULES}:" != *":${util_name_clean}:"* ]]; then
                echo "Utility '${util_name}' is not loaded."
                continue
            fi
...
_complete_DFLOADER() {
    ...
    local commands="load unload list"
